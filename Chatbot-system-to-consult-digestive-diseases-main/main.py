...
def welcome_question():
    print("-->Chatbot: Xin chào, tôi là chatbot chuẩn đoán bệnh cho trẻ từ 3-10 tuổi!")
...
def first_question(list_symptom_of_person, person):
    AllSymLst = [db.resulttrieutrung[0], db.resulttrieutrung[3],
                 db.resulttrieutrung[4], db.resulttrieutrung[26]]
...
def second_question(list_symptom_of_person, person):
    ...
    while (1):
        check = {'idtrieuchung': 'S01', 'noidung': 'Xuất hiện các mảng da đỏ'}
        if (check in list_symptom_of_person):
            print(f'-->Chatbot: {person.name} đang có triệu chứng XUẤT HIỆN CÁC MẢNG DA ĐỎ - một trong số các triệu chứng của bệnh về da. Để có dự đoán chính xác, hãy cho tôi biết chi tiết thêm về hiện tượng của vùng da bị mẩn đỏ đó:')
            print('1. Vùng da đỏ dày lên và ngứa')
            print('2. Xuất hiện các nốt phồng rộp nhỏ đỏ')
            print('0. Hiện tượng khác')
...
def third_question(list_symptom_of_person, person):
    ...
    while (1):
        ...
        if (check in list_symptom_of_person):
            ...
            print(
                f'-->Chatbot: Tiếp theo tôi muốn biết chi tiết hơn về hiện tượng các mảng da đỏ của {person.name}. ')
            count = 1
            for i in Frequency_StomachAcheSymLst:
                if (i not in list_symptom_of_person):
                    print(f'{count}. {i["noidung"]}')
...
def forth_question_before_forward_inference(list_symptom_of_person, person):
   # initTree = TreeForFC('S09', TreeForFC('S14', TreeForFC('S11', TreeForFC('S30'), TreeForFC('S27')), TreeForFC('S22', TreeForFC('S24'), TreeForFC(
    ...
    initTree= TreeForFC('S01',TreeForFC('S08',TreeForFC('S06',TreeForFC('S12'),TreeForFC('S15')),TreeForFC('S05',TreeForFC('S03',TreeForFC('S06')))),TreeForFC('S03',TreeForFC('S10',TreeForFC('S12',TreeForFC('S05',TreeForFC('S15')))),TreeForFC('S05',TreeForFC('S06',TreeForFC('S10',TreeForFC('S12',TreeForFC('S15')))),TreeForFC('S10',TreeForFC('S15',TreeForFC('S12'))))) )
    savedTree = initTree
...
if len(list_predicted_disease)==0 :
    print("Bạn không có dấu hiệu của bệnh nào cả.Cám ơn bạn đã sử dụng ChatBot")
    sys.exit()
